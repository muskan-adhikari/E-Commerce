          ...
          <div className='flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full'>
            {
              productData.image.map((item, idx) => (
                <img onClick={() => setImage(item)} key={idx} src={item} alt='' className='w-[24%] sm:w-full sm:mb-3 flex-shrink-0 cursor-pointer' />
              ))
            }
          </div>
            ...
            <div className='flex gap-2'>
              {
                productData.sizes.map((item, idx) => (
                  <button onClick={() => setSize(item)} key={idx} className={`btn ${item === size ? 'border-orange-500' : ''} `}>{item}</button>
                ))
              }
            </div>
          </div>
